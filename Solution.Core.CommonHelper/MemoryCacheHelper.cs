...
public class MemoryCacheHelper : IMemoryCacheHelper
{
    ...
    private static void InitCacheEntryForMinutes(ICacheEntry entry, int baseExpireSeconds)
    {
        //过期时间.Random.Shared 是.NET6新增的
        double sec = Random.Shared.NextDouble(baseExpireSeconds, baseExpireSeconds * 2);
        TimeSpan expiration = TimeSpan.FromMinutes(sec);
        entry.AbsoluteExpirationRelativeToNow = expiration;
    }
    ...
    public async Task<TResult?> GetOrCreateAsync<TResult>(string cacheKey, Func<ICacheEntry, Task<TResult?>> valueFactory, int baseExpireSeconds = 60,bool isSecond=true)
    {
        ValidateValueType<TResult>();
        if (!memoryCache.TryGetValue(cacheKey, out TResult result))
        {
            ...
            {
                InitCacheEntryForMinutes(entry, baseExpireSeconds);
            }
    ...
    public void Set(string cacheKey, object data,DateTime cacheTime)
    {
