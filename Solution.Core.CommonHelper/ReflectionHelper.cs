...
public static class ReflectionHelper
{
    ...
    public static IEnumerable<Assembly> GetAllReferencedAssemblies(bool skipSystemAssemblies = true)
    {
        ...
        foreach (var asmPath in asmsInBaseDir)
        {
            ...
            //if (asmPath=="E:\\Codes\\Solution.Core\\Solution.Core.WebAdminAPI\\bin\\Debug\\net8.0\\cs\\Microsoft.CodeAnalysis.CSharp.resources.dll")
            if (asmPath.Contains("\\cs\\") || asmPath.Contains("\\de\\") || asmPath.Contains("\\es\\") || asmPath.Contains("\\fr\\") || asmPath.Contains("\\it\\")
                || asmPath.Contains("\\ja\\") || asmPath.Contains("\\ko\\") || asmPath.Contains("\\pl\\") || asmPath.Contains("\\pt-BR\\") || asmPath.Contains("\\ru\\")
                || asmPath.Contains("\\tr\\") || asmPath.Contains("\\zh-Hans\\") || asmPath.Contains("\\zh-Hant\\"))
            {
                continue;
            }
            AssemblyName asmName = AssemblyName.GetAssemblyName(asmPath);
